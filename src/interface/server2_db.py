...
class Camera:
    ...
    def record_video(self):
        ...
        while self.recording:
            ...
            self.insert_video_metadata(filename, timestamp, filepath)
            latest_recordings = get_latest_recordings()
            print("Latest Recordings:", latest_recordings)
            try:
    ...
    def insert_video_metadata(self, filename, timestamp, filepath):
        conn = sqlite3.connect('videos.db')
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO video_metadata (camera_id, filename, timestamp, path)
            VALUES (?, ?, ?, ?)
        ''', (self.camera_id, filename, timestamp, filepath))
        conn.commit()
        conn.close()
